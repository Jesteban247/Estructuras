...
template <class T>
void ArbolAVL<T>::inOrden_vector() 
{
    vector<T> resultado;
    inOrdenRecursivo_vector(raiz, resultado);

    //Calculo de la mediana
    int tamano = resultado.size();
    if (tamano%2 != 0)
    {
        int mediana = resultado[tamano/2];
        cout << "La mediana es: " << mediana << endl;
    }
    else
    {
        float x1 = resultado[tamano/2];
        float x2 = resultado[tamano/2 - 1];
        float mediana = (x1+x2)/2;
        cout << "La mediana es: " << mediana << endl;
    }
}
...
// Métodos privados para operaciones internas
    ...
    template <class T>
    void ArbolAVL<T>::inOrdenRecursivo_vector(NodoAVL<T>* nodo, vector<T> &resultado) 
    {
        if (nodo != nullptr) 
        {
            // Recorrer el subárbol izquierdo
            inOrdenRecursivo_vector(nodo->obtenerHijoIzq(), resultado);
            // Agregar el dato del nodo actual al vector
            resultado.push_back(nodo->obtenerDato());
            // Recorrer el subárbol derecho
            inOrdenRecursivo_vector(nodo->obtenerHijoDer(), resultado);
        }    
