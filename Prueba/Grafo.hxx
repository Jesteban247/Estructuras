...
{
    // Si el nodoInicio no se encuentra en el grafo, retorna un multimap vacío
    if (nodosMap.find(nodoInicio) == nodosMap.end()) {
        cout << "Nodo de inicio no existe en el grafo." << endl;
        return multimap<int, int>();
    }

    // Si el grafo no está conectado, no se puede aplicar el algoritmo de Prim.
    if (!Conectado()) {
    ...
    // Inicializar el nodo de inicio para el algoritmo de Prim
    pesoMinimoHastaNodo[indiceInicio] = 0;
    padre[indiceInicio] = -1;

    for (int count = 0; count < nodos.size() - 1; count++) {
        // Encontrar el nodo con el peso mínimo que no esté incluido aún en el MST
        int u = -1;
        ...
        // Marcar el nodo como incluido
        nodoIncluido[u] = true;

        // Actualizar los valores para los nodos adyacentes
    ...
    // Construir el multimap para el árbol de recubrimiento mínimo
    for (int i = 0; i < nodos.size(); i++) {
        if(padre[i] != -1) {
